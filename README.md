...
- **标题与正文检索**：既可按标题即时筛选，也可按用户/AI 正文关键词全文检索（结果落盘缓存，二次检索无需重读）；可按最近 7/30/90 天或一年限定时间范围
- **干净预览**：只展示真实用户消息与 AI 最终可见正文，代码块与行内代码等宽高亮
- **「只看对话」开关**：一键隐藏思考过程与工具结果，只留问答；导出文件始终完整
...
- 新增「只看对话」开关：默认预览在消息正文为空时会回退显示思考内容，实测一条
  TRAE 对话预览 9358 字符里绝大部分是思考；勾选后只剩 148 字符的真实问答，
  并明确标出「已隐藏 N 条仅含思考/工具的消息」
- 行内 `代码` 与围栏代码块分别用等宽字体和浅底色渲染，代码不再和正文糊成一片
- 消息之间改用留白分隔，时间与模型作为次要信息弱化，长回答更易扫读
- 新增时间范围筛选（今天 / 近 7 天 / 30 天 / 90 天 / 一年）
