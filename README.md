...
预览的目标只有一个：**还原你在那个客户端界面里实际看到的东西**。正文按 Markdown 排版渲染——标题、列表、引用、表格、行内代码、代码块——而不是原样吐标记源码。

难点在于"AI 的回答"存在哪里，每个平台都不一样：

- 有的平台正常写入回答字段——直接渲染。
- 有的平台（agent 型客户端）把最终回答**作为工具调用的参数提交**——界面上显示的是 `finish` 调用里的 `summary` 字段，消息体里根本没有正文。预览会识别这类工具并提取真正的交付（实测某来源 8 条对话 380 次 `finish` 调用，全部是界面上渲染过的回答）。
- 有的平台从不写回答字段，输出整个落在推理里——保留每轮最后一块推理作为结论，否则阅读视图里只剩用户在自言自语。

勾选**只看对话**后，纯机器轮次（思考、工具往返）整条隐藏。实测一条 242 万字符的对话，阅读视图收敛到 5.7 万字符，AI 的话一句没少。
...
**回答可能藏在工具调用里。** agent 型客户端的"最终回答"往往是一次工具调用的参数（`finish.summary`、`attempt_completion.result`）。预览层维护一张答案工具注册表，按工具名+字段提取；不在表里的工具一律视为机器动作。同理，存储层的 `part_type` 拼法不能想当然——某来源写库用的是 `'tool'` 而不是 `'tool_call'`，旧解析让 14,000+ 条工具调用整体消失。
...
166 个测试。数据完整性相关的缺陷一律要求附带能复现它的回归测试——"导出必须全量"是这个项目的核心约束，约束需要测试来守。
